...
export default function SubscriptionPage() {
  ...
  const handleSelectPlan = async (planKey: PlanKey) => {
    if (!user || !user.email) {
        toast({ variant: 'destructive', title: 'Authentication Error', description: 'Could not find user details. Please log in again.' });
        return;
    }
    if (planKey === subscription?.plan) return;
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(plans).filter(([key]) => key !== 'None').map(([key, plan]) => {
                ...
                const isProcessingThisPlan = isProcessing === planKey;
                ...
                return (
                    <Card key={key} className={`flex flex-col ${isCurrent ? 'border-primary border-2' : ''}`}>
                        ...
                        <CardFooter>
                            <Button className="w-full" onClick={() => handleSelectPlan(planKey)} disabled={isCurrent || isProcessing}>
                                {isProcessingThisPlan ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                                {isCurrent ? 'Current Plan' : isProcessingThisPlan ? 'Processing...' : plan.cta}
                            </Button>
                        </CardFooter>
                    </Card>
                )
